...
#             store_index = depth - compval
#             if 0 <= store_index < len(self.valnum):
#                 prev_val = self.valnum[store_index]

#                 # Strict increasing-order check
#                 if not self._check_increasing_order(store_index, valueType2, prev_val, v2):
#                     self.logger.warning(
#                         f"Out-of-order sequence at depth {store_index}: expected after {prev_val}, got {v2}. Token skipped."
#                     )
#                     return None  # skip this token completely

#                 # update state only if order is valid
...
#     def _check_increasing_order(self, depth, valueType, prev_val, curr_val):
#         """
#         Universal increasing-order enforcement at the given depth.
#         Returns:
#             True  -> valid increasing order
#             False -> out-of-order sequence
#         """
#         try:
#             if not prev_val:
...
#             # dotted decimals
#             if self._is_dotted_decimal(prev_val) and self._is_dotted_decimal(curr_val):
#                 prev_parts = [int(x) for x in prev_val.split('.')]
#                 curr_parts = [int(x) for x in curr_val.split('.')]
#                 if len(prev_parts) == len(curr_parts):
#                     return curr_parts[:-1] == prev_parts[:-1] and curr_parts[-1] == prev_parts[-1] + 1
#                 return True

#             # flat sequences: decimal / roman / alphabet
#             next_map = self.nextvals.get(valueType, {})
#             if prev_val in next_map and curr_val in next_map:
#                 return next_map[curr_val] == next_map[prev_val] + 1

#             return True
#         except Exception as e:
#             self.logger.error(f"Order check failed at depth {depth}: {e}")
#             return True